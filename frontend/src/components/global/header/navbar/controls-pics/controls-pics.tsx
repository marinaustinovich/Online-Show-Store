...
export const ControlsPics = ({ onSearchExpanderClick }: ControlsPicsProps) => {
    ...
    return (
        <div className={cn('pics')}>
            <div data-id='search-expander' className={cn('pic', [cn('search')])} onClick={onSearchExpanderClick}></div>

            <div className={cn('pic', [cn('cart')])} onClick={() => navigate('/cart')}>
                {selectedProductsCount > 0 && <div className={cn('cart-full')}>{selectedProductsCount}</div>}
