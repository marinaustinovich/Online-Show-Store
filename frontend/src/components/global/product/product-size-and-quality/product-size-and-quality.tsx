...
export const ProductSizeAndQuantity = () => {
    ...
    const sizes = useMemo(() => (productSizes ? productSizes.filter(size => size.available).map(size => size.size) : []), [productSizes]);

    const handleSizeSelect = useCallback((value: string) => setSelectedSize(value), []);

    const handleQuantityChange = useCallback((newQuantity: number) => setSelectedQuantity(newQuantity), []);
    ...
    const showQuantitySelectorAndButton = useMemo(() => sizes.length > 0, [sizes]);

    return (
        <>
            <div className='text-center'>
                <RadioButtonsGroup label={t('product.sizes-label')} options={sizes} onSelect={handleSizeSelect} selectedOption={selectedSize} />
                {showQuantitySelectorAndButton && (
                    <QuantitySelector maxQuantity={10} label={t('product.count-label')} onQuantityChange={handleQuantityChange} />
                )}
            </div>
            {showQuantitySelectorAndButton && (
                <Button disabled={!selectedSize} className='btn-danger btn-block btn-lg' onClick={handleProductBuy}>
