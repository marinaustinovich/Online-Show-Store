...
export const CartTable = ({ cart }: CartTableProps) => {
    ...
    const columns = useMemo(
        () => [
            { header: t(`${locale}.number`), accessor: 'number' },
            {
                ...
                accessor: 'title',
                render: (data: { id: string; name: string }) => <a href={`/products/${data.id}`}>{data.name}</a>,
            },
            ...
            { header: t(`${locale}.total`), accessor: 'total' },
            {
                ...
                render: (data: { id: number }) => (
                    <Button className='btn-outline-danger btn-sm' onClick={() => handleProductDelete(data.id)}>
                        {t(`${locale}.delete`)}
                    </Button>
                ),
            },
    ...
    const totalPrice = useMemo(() => cart.reduce((sum, item) => sum + item.total, 0), [cart]);
    ...
    const footer = useMemo(
        () => (
            <tr>
                ...
                <td>{`${formatPrice(totalPrice)} ${t(`${locale}.ru-currency-label`)}`}</td>
            </tr>
        ),
        [totalPrice, t],
    );

    return <Table data={cartData} columns={columns} includeHead={true} footer={footer} />;
};
