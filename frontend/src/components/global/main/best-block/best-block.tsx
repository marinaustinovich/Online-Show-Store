...
export const BestBlock = () => {
    ...
    const showComponent = useMemo(() => topSalesStatus === RequestStatus.PROCESSING || topSalesProducts.length > 0, [topSalesProducts.length, topSalesStatus]);

    return showComponent ? (
        <TopSalesSection>
            <Title text={t('main.best.title')} />
            {topSalesStatus === RequestStatus.PROCESSING && <Preloader />}
            <Row>{topSalesProducts?.map(product => <Card card={product} key={product.id} />)}</Row>
        </TopSalesSection>
    ) : null;
};
